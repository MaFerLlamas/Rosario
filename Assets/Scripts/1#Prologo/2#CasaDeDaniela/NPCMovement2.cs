...
public class NPCMovement2 : MonoBehaviour
{
    ...
    private void nextPosition()
    {
        ...
        if (positions.Length > 1)
        {
            ...
            {//Cuidando no salirnos del array
                if (isCicled)
                { // pero si el movimiento es repetitivo volvemos al inicio
                    pos = 0;
    ...
    private void changeAnimationDirection()
    {
        ...
        {
            if (currentTarget.y > currentStart.y)
        ...
        else if (currentTarget.y == currentStart.y)
        { //Si no hay diferencia en Vertical entonces se mueve hacia los lados
            if (currentTarget.x < currentStart.x)
